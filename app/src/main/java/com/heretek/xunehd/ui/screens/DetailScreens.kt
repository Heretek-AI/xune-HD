...
fun AlbumDetailScreen(albumId: Long, canvasWidth: androidx.compose.ui.unit.Dp) {
    ...
    LaunchedEffect(albumId) {
        ...
        album = tracks.firstOrNull()?.let {
            com.heretek.xunehd.data.model.Album(
                albumId = it.albumId,
                title = it.album,
                artist = it.artist,
...
private fun ArtistAlbums(albums: List<Album>) {
    val graph = LocalXuneGraph.current
    LazyRow(
        contentPadding = androidx.compose.foundation.layout.PaddingValues(XuneTokens.EDGE.dp),
...
private fun ArtistSongs(
    tracks: List<Track>,
    menus: com.heretek.xunehd.ui.components.MenuController,
    graph: com.heretek.xunehd.XuneGraph,
) {
    val nowPlayingId by graph.controller.nowPlaying.collectAsState()
    KineticList(
        items = tracks,
