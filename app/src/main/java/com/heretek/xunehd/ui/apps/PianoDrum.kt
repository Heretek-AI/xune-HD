...
fun PianoApp() {
    ...
    DetailScaffold(title = "piano") {
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Box(Modifier.fillMaxWidth().height(XuneTokens.PIANO_KEY_H.dp).background(colors.elevated)) {
                Row(Modifier.fillMaxSize()) {
                    repeat(14) { i ->
                        Box(
                            Modifier
                                .size(width = XuneTokens.PIANO_KEY_W.dp, height = XuneTokens.PIANO_KEY_H.dp)
