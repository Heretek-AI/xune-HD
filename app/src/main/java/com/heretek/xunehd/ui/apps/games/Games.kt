...
fun SolitaireApp() {
    ...
    DetailScaffold(title = "solitaire") {
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            Row(verticalAlignment = androidx.compose.ui.Alignment.CenterVertically, modifier = Modifier.fillMaxWidth()) {
                BasicText(
                    text = if (stock.cards.isEmpty()) "recycle" else "deal",
                    style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_LIST.sp, color = colors.accent),
                    modifier = Modifier.pointerInput(Unit) { detectTapGestures(onTap = { tapStock() }) }.padding(end = 12.dp),
                )
                BasicText(
                    text = "waste ${waste.cards.size}",
                    style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_CAPTION.sp, color = colors.textSecondary),
                )
            }
...
private fun CardView(card: SolCard, onClick: () -> Unit) {
    ...
    Box(
        Modifier
            .size(width = XuneTokens.CARD_W.dp, height = XuneTokens.CARD_H.dp)
            .background(bg)
            .padding(2.dp)
            ...
            for (r in 0 until 9) {
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(1.dp)) {
                    for (c in 0 until 9) {
                        ...
                        Box(
                            Modifier
                                .size(width = XuneTokens.SUDOKU_CELL.dp, height = XuneTokens.SUDOKU_CELL.dp)
                                .background(
                                    when {
                                        isSel -> colors.accent
                                        isGiven -> colors.tile
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(4.dp)) {
                (1..9).forEach { n ->
                    Box(
                        ...
                        BasicText(text = n.toString(), style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_LIST.sp, color = colors.textPrimary))
                    }
                }
                Box(
                    Modifier
                    ...
                    BasicText(text = "x", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_LIST.sp, color = colors.textSecondary))
                }
            }
        }
    }
...
fun HexicApp() {
    ...
    DetailScaffold(title = "hexic") {
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp)) {
            BasicText(text = "score $score  best ${bestFlow.firstOrNull()?.score ?: 0}", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_NOW_META.sp, color = colors.accent))
            Spacer(Modifier.height(8.dp))
            val hexicAccent = LocalXuneColors.current.accent
                    ...
                    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(2.dp)) {
                        for (c in 0 until n) {
                            Box(
                                Modifier
                                    .weight(1f)
                                    .height(XuneTokens.HEX_RADIUS.dp * 3)
            ...
            Spacer(Modifier.height(8.dp))
            BasicText(text = "+ rotate tile • clear 3+ same-color adjacents", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_CAPTION.sp, color = colors.textSecondary))
        }
    }
}
...
fun ReversiApp() {
    ...
    DetailScaffold(title = "reversi") {
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp)) {
            BasicText(text = "you $bs — ai $ws   ${if (player == ReversiEngine.BLACK) "your move" else "ai…"}", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_NOW_META.sp, color = colors.textPrimary))
            Spacer(Modifier.height(8.dp))
            for (r in 0 until 8) {
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(1.dp)) {
                    for (c in 0 until 8) {
                        ...
                        Box(
                            ...
                            if (v != ReversiEngine.EMPTY) {
                                ...
                                BasicText(text = "·", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_LIST.sp, color = colors.accent))
                            }
                        }
                    }
                }
