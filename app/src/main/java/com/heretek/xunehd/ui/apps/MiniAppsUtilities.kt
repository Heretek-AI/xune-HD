        ...
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp)) {
            BasicText(
                ...
                style = TextStyle(
                    ...
                    fontSize = XuneTokens.TYPE_HEADER_CROP_VISIBLE.sp * 1.4f,
        ...
        DetailScaffold(title = "notes") {
            Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp)) {
                ...
                Box(Modifier.fillMaxWidth().combinedClickable(
                    ...
                    EdgeCropText(text = "save", fontSize = XuneTokens.TYPE_LIST.dp, color = LocalXuneColors.current.accent)
                }
                Spacer(Modifier.height(4.dp))
                Box(Modifier.fillMaxWidth().combinedClickable(
                    onClick = {
                    ...
                    EdgeCropText(text = "delete", fontSize = XuneTokens.TYPE_LIST.dp, color = LocalXuneColors.current.textInactive)
                }
            }
        }
        return
    ...
    DetailScaffold(title = "notes") {
        Column(Modifier.fillMaxSize()) {
            Box(
                ...
                EdgeCropText(text = "+ new note", fontSize = XuneTokens.TYPE_LIST.dp, color = LocalXuneColors.current.accent)
            }
            KineticList(
                items = notes,
                key = { it.id },
                ...
                rowContent = { n, _ ->
                    Box(
                        ...
                        EdgeCropText(text = n.title, fontSize = XuneTokens.TYPE_LIST.dp, modifier = Modifier.fillMaxWidth())
                    }
                },
            )
        }
...
fun EditableLine(
    ...
    androidx.compose.foundation.text.BasicTextField(
        ...
        textStyle = TextStyle(
            fontFamily = Selawik,
            fontSize = if (multiLine) XuneTokens.TYPE_LIST.sp else XuneTokens.TYPE_NOW_META.sp,
    ...
    if (local.isEmpty() && placeholder.isNotEmpty()) {
        EdgeCropText(text = placeholder, fontSize = XuneTokens.TYPE_NOW_META.dp, alpha = 0.4f, modifier = Modifier.padding(start = 8.dp, top = 4.dp))
    }
}
    ...
    DetailScaffold(title = "stopwatch") {
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp)) {
            ...
            laps.asReversed().forEach { lap ->
                EdgeCropText(text = formatTime(lap / 1000), fontSize = XuneTokens.TYPE_LIST.dp, alpha = 0.7f, modifier = Modifier.fillMaxWidth().padding(vertical = 2.dp))
            }
        }
    }
}
        ...
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp)) {
            BasicText(
                ...
                style = TextStyle(
                    fontFamily = Selawik,
                    fontWeight = FontWeight.Light,
                    fontSize = XuneTokens.TYPE_HEADER_CROP_VISIBLE.sp * 2,
                    color = colors.textPrimary,
                ),
                modifier = Modifier.fillMaxWidth(),
            )
            ...
            Spacer(Modifier.height(8.dp))
            EdgeCropText(text = "beat $beat", fontSize = XuneTokens.TYPE_LIST.dp, alpha = 0.6f, modifier = Modifier.fillMaxWidth())
        ...
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp)) {
            Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                ChordData.ROOTS.forEach { r ->
                    EdgeCropText(text = r, fontSize = XuneTokens.TYPE_LIST.dp, color = if (r == root) LocalXuneColors.current.accent else LocalXuneColors.current.textPrimary,
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                ChordData.QUALITIES.forEach { q ->
                    EdgeCropText(text = q, fontSize = XuneTokens.TYPE_LIST.dp, color = if (q == quality) LocalXuneColors.current.accent else LocalXuneColors.current.textPrimary,
...
fun ShuffleByAlbumApp() {
    ...
    var pick by remember { mutableStateOf<com.heretek.xunehd.data.model.Album?>(null) }
    ...
    DetailScaffold(title = "shuffle by album") {
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            pick?.let { album ->
                com.heretek.xunehd.design.components.AlbumArt(
                    model = album.albumArtUri,
                    contentDescription = album.title,
                    modifier = Modifier.size(120.dp),
                )
                EdgeCropText(text = album.title, fontSize = XuneTokens.TYPE_NOW_META.dp)
                EdgeCropText(text = album.artist, fontSize = XuneTokens.TYPE_LIST.dp, color = colors.textSecondary)
                EdgeCropText(
                    text = "play",
                    fontSize = XuneTokens.TYPE_LIST.dp,
                    color = colors.accent,
...
fun MusicQuizApp() {
    ...
    DetailScaffold(title = "music quiz") {
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp)) {
            val q = question
            if (q == null) {
                EdgeCropText(text = "loading…", fontSize = XuneTokens.TYPE_NOW_META.dp, alpha = 0.4f)
                return@Column
            }
            BasicText(
                text = q.prompt,
                style = TextStyle(
                    fontFamily = Selawik,
                    fontWeight = FontWeight.Light,
                    fontSize = XuneTokens.TYPE_HEADER_CROP_VISIBLE.sp * 1.2f,
            ...
            Spacer(Modifier.height(8.dp))
            EdgeCropText(text = "correct $correct — wrong $wrong", fontSize = XuneTokens.TYPE_CAPTION.dp, alpha = 0.6f)
        }
    }
}
...
fun AlarmClockApp() {
    ...
    fun schedule(alarm: com.heretek.xunehd.data.db.AlarmEntity) {
        val am = context.getSystemService(android.content.Context.ALARM_SERVICE) as android.app.AlarmManager
        val triggerAt = com.heretek.xunehd.media.AlarmScheduler.nextFireMs(alarm.hour, alarm.minute)
        val pi = android.app.PendingIntent.getBroadcast(
            context,
            alarm.id.toInt(),
            android.content.Intent(context, com.heretek.xunehd.media.AlarmReceiver::class.java).apply {
                        ...
                        onClick = {
                            scope.launch {
                                val id = graph.alarms.add(
                                    com.heretek.xunehd.data.db.AlarmEntity(
                                        hour = 7, minute = 0, enabled = true,
                                        label = "alarm", alarmKind = 0, refId = 0, daysOfWeek = 0,
                                    ),
                                )
                                schedule(com.heretek.xunehd.data.db.AlarmEntity(id = id, hour = 7, minute = 0, enabled = true, label = "alarm", alarmKind = 0, refId = 0, daysOfWeek = 0))
                            }
                        },
                        onLongClick = {},
                    ),
            ) {
                EdgeCropText(text = "+ new alarm", fontSize = XuneTokens.TYPE_LIST.dp, color = LocalXuneColors.current.accent)
            }
            KineticList(
                items = alarms,
                key = { it.id },
                    ...
                    Row(
                                        ...
                                        if (!alarm.enabled) {
                                            ...
                                            am.cancel(
                                                android.app.PendingIntent.getBroadcast(
                                                    context,
                                                    alarm.id.toInt(),
                                                    android.content.Intent(context, com.heretek.xunehd.media.AlarmReceiver::class.java),
                                                    android.app.PendingIntent.FLAG_UPDATE_CURRENT or android.app.PendingIntent.FLAG_IMMUTABLE,
                                                ),
                                            )
                                        } else schedule(alarm)
                        ...
                        Column(Modifier.weight(1f)) {
                            EdgeCropText(
                                ...
                                color = if (alarm.enabled) LocalXuneColors.current.textPrimary else LocalXuneColors.current.textInactive,
                            )
                            EdgeCropText(text = alarm.label, fontSize = XuneTokens.TYPE_LIST.dp, color = LocalXuneColors.current.textSecondary)
                        }
                        EdgeCropText(text = if (alarm.enabled) "on" else "off", fontSize = XuneTokens.TYPE_CAPTION.dp, color = LocalXuneColors.current.textSecondary)
                    }
                },
            )
        }
    ...
    DetailScaffold(title = "calendar") {
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxWidth()) {
                EdgeCropText(text = "<", fontSize = XuneTokens.TYPE_NOW_META.dp, color = LocalXuneColors.current.accent,
                    modifier = Modifier.combinedClickable(onClick = { monthStart = monthStart.minusMonths(1) }, onLongClick = {}))
                EdgeCropText(text = "%s %d".format(monthStart.month.name.lowercase().replaceFirstChar { it.uppercase() }, monthStart.year), fontSize = XuneTokens.TYPE_NOW_META.dp,
...
@Composable
private fun MonthGrid(monthStart: java.time.YearMonth, appts: List<com.heretek.xunehd.data.db.AppointmentEntity>) {
