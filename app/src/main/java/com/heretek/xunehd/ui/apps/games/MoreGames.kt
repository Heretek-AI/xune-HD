package com.heretek.xunehd.ui.apps.games
...
fun HeartsApp() {
    ...
    DetailScaffold(title = "hearts") {
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp)) {
            BasicText(text = "scores: ${state.scores.entries.joinToString { "${it.key.name.lowercase()}=${it.value}" }}", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_LIST.sp, color = colors.textSecondary))
            ...
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceEvenly) {
                state.trick.forEach { (s, c) -> Column { BasicText(s.name.lowercase(), style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_CAPTION.sp, color = colors.textSecondary)); SolCardView(c) {} } }
            }
            Spacer(Modifier.height(8.dp))
            PlayerHand(
            ...
            if (passCards) {
                Spacer(Modifier.height(8.dp))
                BasicText(text = "pass 3 cards", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_LIST.sp, color = colors.accent))
                PassPicker(hand = state.hands[seat]!!, onConfirm = { cards -> state = HeartsEngine.passCards(state, seat, cards) })
            }
            if (state.done) {
                Spacer(Modifier.height(8.dp))
                BasicText(text = "round complete", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_NOW_META.sp, color = colors.accent))
            }
        }
        if (state.done) {
...
private fun PlayerHand(hand: List<SolCard>, onCard: (SolCard) -> Unit = {}, align: Alignment.Horizontal, label: String, interactive: Boolean = true) {
    Column(horizontalAlignment = align) {
        BasicText(text = label, style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_CAPTION.sp, color = LocalXuneColors.current.textSecondary))
...
private fun PassPicker(hand: List<SolCard>, onConfirm: (List<SolCard>) -> Unit) {
    ...
    Column {
        ...
        BasicText(text = "pass ${picked.size}/3 — ${if (picked.size == 3) "tap confirm" else "pick three"}", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_LIST.sp, color = LocalXuneColors.current.accent),
...
fun SpadesApp() {
    ...
    DetailScaffold(title = "spades") {
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp)) {
            BasicText(text = "score ${state.score.first} / ${state.score.second}", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_NOW_META.sp, color = colors.accent))
            Spacer(Modifier.height(4.dp))
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            ...
            if (!state.biddingDone) {
                if (state.currentPlayer == seat) {
                    BasicText(text = "your bid:", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_LIST.sp, color = colors.textPrimary))
                    Row(horizontalArrangement = Arrangement.spacedBy(4.dp)) {
                        (1..13).forEach { n ->
                            BasicText(text = "$n", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_LIST.sp, color = colors.accent),
                                modifier = Modifier.pointerInput(n) { detectTapGestures(onTap = { human(state) { SpadesEngine.bid(it, n) } }) })
                        }
                    }
                } else {
                    BasicText(text = "ai bidding…", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_LIST.sp, color = colors.textSecondary))
                }
            } else {
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceEvenly) {
                    state.trick.forEach { (s, c) -> Column { BasicText(s.name.lowercase(), style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_CAPTION.sp, color = colors.textSecondary)); SolCardView(c) {} } }
                ...
                if (state.currentPlayer == seat) {
                    ...
                    BasicText(text = "ai playing…", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_LIST.sp, color = colors.textSecondary))
                }
            }
            Spacer(Modifier.height(4.dp))
...
private fun SpadesBidSeat(state: SpadesState, seat: SpadesSeat, viewer: SpadesSeat, onInteract: () -> Unit) {
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        BasicText(text = "${seat.name.lowercase()}: ${state.bids[seat]}", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_CAPTION.sp, color = LocalXuneColors.current.textSecondary))
...
fun CheckersApp() {
    ...
    DetailScaffold(title = "checkers") {
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp)) {
            BasicText(text = if (state.winner != null) "${state.winner} wins" else "${state.turn} move", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_NOW_META.sp, color = colors.accent))
            Spacer(Modifier.height(8.dp))
            ...
            Spacer(Modifier.height(8.dp))
            Row {
                BasicText(text = "you are red", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_CAPTION.sp, color = colors.textSecondary))
            }
        }
    }
...
fun ChessApp() {
    ...
    DetailScaffold(title = "chess") {
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp)) {
            BasicText(text = if (state.status.isNotEmpty()) state.status else if (state.turn == ChessColor.WHITE) "your move" else "ai thinking…", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_NOW_META.sp, color = colors.accent))
            Spacer(Modifier.height(4.dp))
            ...
            Spacer(Modifier.height(4.dp))
            BasicText(text = "tap a piece, then a target", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_CAPTION.sp, color = colors.textSecondary))
        }
    }
...
fun TexasHoldemApp() {
    ...
    DetailScaffold(title = "texas hold 'em") {
        Column(Modifier.fillMaxSize().padding(XuneTokens.EDGE.dp)) {
            BasicText(text = "pot ${state.pot} • you ${state.playerStack} • ai ${state.aiStack} • ${state.phase}", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_LIST.sp, color = colors.accent))
            Spacer(Modifier.height(8.dp))
            BasicText(text = "ai: ${state.aiHole.joinToString("") { it.toString().take(2) }}", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_CAPTION.sp, color = colors.textSecondary))
            Spacer(Modifier.height(4.dp))
            Row(horizontalArrangement = Arrangement.spacedBy(4.dp)) {
                state.community.forEach { SolCardView(it) {} }
            }
            Spacer(Modifier.height(8.dp))
            BasicText(text = "you: ${state.playerHole.joinToString("") { it.toString().take(2) }}", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_NOW_META.sp, color = colors.textPrimary))
            Row(horizontalArrangement = Arrangement.spacedBy(4.dp)) {
                state.playerHole.forEach { SolCardView(it) {} }
            }
            Spacer(Modifier.height(8.dp))
            if (state.phase == PokerPhase.SHOWDOWN || state.phase == PokerPhase.DONE) {
                BasicText(text = if (state.winner == "tie") "tie" else "${state.winner} wins", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_NOW_META.sp, color = colors.accent))
                BasicText(text = "new hand", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_LIST.sp, color = colors.accent),
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    legal.forEach { a ->
                        BasicText(text = a.name.lowercase(), style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_LIST.sp, color = colors.accent),
                            modifier = Modifier.pointerInput(a) { detectTapGestures(onTap = { state = PokerEngine.applyAction(state, a) }) })
                    }
                }
            } else {
                BasicText(text = "ai thinking…", style = TextStyle(fontFamily = Selawik, fontSize = XuneTokens.TYPE_LIST.sp, color = colors.textSecondary))
            }
        }
    }
