...
import com.heretek.xunehd.MainActivity
import com.heretek.xunehd.XuneApp
 ...
 */
class XunePlaybackService : MediaSessionService() {
    ...
    private fun resolveMediaItems(requested: List<MediaItem>): com.google.common.util.concurrent.ListenableFuture<MutableList<MediaItem>> =
        GlobalScope.future(Dispatchers.IO) {
            val repo = (applicationContext as XuneApp).graph.library
            requested.map { item ->
